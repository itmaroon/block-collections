...
export default function Edit({
	...
	//ラベルの参照
	const labelRef = useRef<HTMLLabelElement | null>(null); //レンダリングで参照の設定を忘れないこと
	...
	const mergeBoxValues = (
		current: typeof default_pos.margin,
		value: BoxControlValue,
	): typeof default_pos.margin => ({
		...current,
		...Object.fromEntries(
	...
	const handleResponsive = (
		property: keyof typeof default_pos,
		value:
			| RadioDirection
			| RadioAlign
			| boolean
		...
		const currentPos = isMobile ? mobile_pos : default_pos;
		const nextValue =
			property === "margin" ||
			property === "padding" ||
			property === "margin_input" ||
			property === "padding_input"
				? mergeBoxValues(currentPos[property], value as BoxControlValue)
				: value;
		if (!isMobile) {
			setAttributes({
				default_pos: { ...default_pos, [property]: nextValue },
			});
		} else {
			setAttributes({
				mobile_pos: { ...mobile_pos, [property]: nextValue },
			});
		}
	};
				...
				{isSetSelect && (
					...
					>
						<OptionModal
							...
							onAddOption={(selectedOption: RadioOption) => {
								setAttributes({
									optionValues: [...optionValues, selectedOption],
								});
							}}
							onUpdateOption={(updatedValues: RadioOption[]) => {
			...
			<InspectorControls group="styles">
					...
					>
						<BorderBoxControl
							onChange={(newValue: Record<string, any> | undefined) =>
								setAttributes({ border_box: newValue })
							}
							value={border_box}
							allowReset={true} // リセットの可否
				...
				>
						...
						<>
							...
							<UnitControl
								...
								onChange={(value: string | undefined) =>
									handleResponsive("button_scale", value)
								}
					...
					<BoxControl
						...
						onChange={(value: BoxControlValue) =>
							handleResponsive("margin_input", value)
						}
						units={units} // 許可する単位
						allowReset={true} // リセットの可否
						resetValues={padding_resetValues} // リセット時の値
					...
					<BoxControl
						...
						onChange={(value: BoxControlValue) =>
							handleResponsive("padding_input", value)
						}
						units={units} // 許可する単位
						allowReset={true} // リセットの可否
						resetValues={padding_resetValues} // リセット時の値
