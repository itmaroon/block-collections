...
const escapeCssContent = (value) =>
	String(value ?? "")
		.replace(/\\/g, "\\\\")
		.replace(/"/g, '\\"')
		.replace(/\r?\n/g, "\\A ");

const adjustPadding = (padding, posX, posY, space) => {
...
const createOptionStyleCss = (attributes, scope) => {
	...
	const [beforePosition, afterPosition] =
		alignMap[alignment_copy] || alignMap["top left"];
	...
	const copySpace =
		alignment_copy.split(" ")[0] === "center" ? horizontalSpace : verticalSpace;
 ...
 */
export const createTitleStyleCss = (attributes, scope) => {
	const {
		headingType = "H2",
