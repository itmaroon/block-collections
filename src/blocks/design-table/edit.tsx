...
export default function Edit({
	...
	//テーブルの選択カラム
	const [selCulumn, setSelCulumn] = useState<number>(0);
	...
	useEffect(() => {
		...
		if (!is_data_form) {
			...
			targetCols = Math.max(
				...tableSource.map((row: TableRow) =>
					row.cells ? row.cells.length : 0,
				),
			);
			targetRows = Math.max(
				...tableSource.map((col: TableRow) =>
					col.cells ? col.cells.length : 0,
				),
			);
		}
		...
		const setHeading = (
			newHeading: string[],
			targetCells: number,
			attrName: "tableHeading" | "rowTableHeading",
		) => {
	...
	const handleMouseDown = useCallback(
		(index: number) => (e: ReactMouseEvent<HTMLElement>) => {
			...
			const cols =
				tableElement.querySelectorAll<HTMLTableColElement>("colgroup col");
												...
												return (
													<CellTag
														...
														style={{
															...
															textAlign: columAlign[colIndex] as CssTextAlign,
															...cell.style,
														}}
				...
				>
					<ToggleControl
						...
						onChange={(newValue: boolean) =>
							setAttributes({ is_data_form: newValue })
						}
					/>
					{is_data_form && (
						<TextControl
					...
					<ToggleControl
						...
						checked={is_heading}
						onChange={(newValue: boolean) =>
							setAttributes({ is_heading: newValue })
						}
					/>
					<ToggleControl
						...
						onChange={(newValue: boolean) =>
							setAttributes({ is_rowHeading: newValue })
						}
					/>
				</PanelBody>
			</InspectorControls>
			...
			<InspectorControls group="styles">
				...
				>
					<TypographyControls
						...
						onChange={(newStyle: DesignTableAttributes["font_style_th"]) => {
						...
						onChange={(value: BoxControlValue) => {
							if (!isMobile) {
								setAttributes({
									default_pos: {
										...default_pos,
										padding_th: mergeBoxValues(default_pos.padding_th, value),
									},
								});
							} else {
								setAttributes({
									mobile_pos: {
										...mobile_pos,
										padding_th: mergeBoxValues(mobile_pos.padding_th, value),
									},
								});
							}
						}}
				...
				>
					<TypographyControls
						...
						onChange={(newStyle: DesignTableAttributes["font_style_td"]) => {
						...
						onChange={(value: BoxControlValue) => {
							if (!isMobile) {
								setAttributes({
									default_pos: {
										...default_pos,
										padding_td: mergeBoxValues(default_pos.padding_td, value),
									},
								});
							} else {
								setAttributes({
									mobile_pos: {
										...mobile_pos,
										padding_td: mergeBoxValues(mobile_pos.padding_td, value),
									},
								});
							}
						}}
			...
			<BlockControls>
				<AlignmentToolbar
					...
					onChange={(nextAlign: string | undefined) => {
						const align = nextAlign ?? "left";
						if (isSelHeader) {
							setAttributes({ columAlignTH: align });
						} else {
							const newColumAlign = [...columAlign];
							newColumAlign[selCulumn] = align;
