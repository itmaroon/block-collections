...
const StyledDiv = styled.div<{ $attr: DesignButtonAttributes }>`
	${({ $attr }) => {
		const {
			buttonType,
			displayType,
		...
		if (is_shadow && shadow_result) {
			...
			if (hover_elm?.style) {
				hover_shadow_style = convertToScss(toStyleRecord(hover_elm.style));
			}
		}
		//擬似要素
		const arrow = Arrow({ direction: pseudoInfo.option as ArrowDirection });
