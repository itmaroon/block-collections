...
import "./editor.scss";
import type {
	CodeHighlightAttributes,
	CodeHighlightEditProps,
} from "./types";
...
const mergeBoxValues = (
	current: CodeHighlightAttributes["margin_value"],
	value: BoxControlValue,
): CodeHighlightAttributes["margin_value"] => ({
	...current,
	...Object.fromEntries(
...
export default function Edit({ attributes, setAttributes }: CodeHighlightEditProps) {
	const {
		codeArea,
		linenums,
		linenumsStart,
	...
	const getPreview = (): ReactNode => {
		...
		return (
			<div className={"wp-block-itmar-code-highlight" + add_block_class}>
	...
	// プレビューのセット
	const [codePreview, setCodePreview] = useState<ReactNode>(getPreview());
	...
	return (
		<>
				...
				>
					<PanelRow>
						<ToggleControl
							label={
								linenums
									? __("Line Number(display)", "block-collections")
									: __("Line Number(hidden)", "block-collections")
							}
							checked={linenums}
							onChange={(val: boolean) => setAttributes({ linenums: val })}
						/>
					</PanelRow>
					{linenums && ( //上記が true の場合に表示
						<PanelRow>
							<TextControl
								...
								value={linenumsStart}
							onChange={(val: string) =>
								setAttributes({ linenumsStart: parseInt(val, 10) || 1 })
							}
							/>
						</PanelRow>
					)}
					...
					<PanelRow>
						<SelectControl
							...
							onChange={(val: string) => setAttributes({ lang: val })}
						/>
					</PanelRow>
					...
					<PanelRow>
						<SelectControl
							...
							onChange={(val: string) => setAttributes({ skin: val })}
						/>
					</PanelRow>
				</PanelBody>
				<PanelBody
				...
				>
					<BoxControl
						...
						onChange={(value: BoxControlValue) =>
							setAttributes({
								margin_value: mergeBoxValues(margin_value, value),
							})
						}
						units={units} // 許可する単位
					...
					<BoxControl
						...
						onChange={(value: BoxControlValue) =>
							setAttributes({
								padding_value: mergeBoxValues(padding_value, value),
							})
						}
						units={units} // 許可する単位
						allowReset={true} // リセットの可否
			...
			<BlockControls>
				<Toolbar label={__("Code highlight tools", "block-collections")}>
					<Button
						//属性 isEditMode の値により表示するラベルを切り替え
						label={
							isEditMode
								? __("Preview", "block-collections")
								: __("Edit", "block-collections")
						}
			...
			{isEditMode && ( // isEditMode が true の場合（編集モード）
				<div {...blockProps}>
					<TextControl
						...
						className="filename"
						value={fileName}
						onChange={(val: string) => setAttributes({ fileName: val })}
					/>
					<TextareaControl
