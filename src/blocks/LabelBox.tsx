...
import type { LabelBoxProps } from "../shared/types";
					...
					<PanelRow className="labelInfo_row">
						<TextControl
							...
							onChange={(newValue: string) =>
								props.onChange("labelContent", newValue)
							}
						/>
					</PanelRow>
					<label>
				...
				>
					...
					>
						<BorderBoxControl
							onChange={(newValue: Record<string, any> | undefined) =>
								props.onChange("border_label", newValue)
							}
							value={border_label}
					...
					<BoxControl
						...
						onChange={(value: BoxControlValue) =>
							props.onChange("padding_label", value)
						}
						units={units} // 許可する単位
						allowReset={true} // リセットの可否
					...
					<UnitControl
						dragDirection="e"
						onChange={(newValue: string | undefined) =>
							props.onChange("labelSpace", newValue)
						}
