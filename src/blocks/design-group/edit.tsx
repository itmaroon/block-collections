...
interface BlockEditorSelectors {
	...
	getBlocksByClientId: (clientId: string) => BlockInstance[];
...
function checkInnerGroupBlocks(blocks: BlockInstance[]): boolean {
...
function checkSubmenuBlocks(blocks: BlockInstance[] = []): boolean {
	for (const block of blocks) {
...
const getParallaxRange = (
	...
	if (key in paraValueMap) {
		return paraValueMap[key as keyof typeof paraValueMap];
	}

	return paraValueMap.x;
};
...
export default function Edit(props: GroupEditProps) {
	...
	const handleHambergerToggle = (isOpen: boolean): void => {
	...
	const handlePositionChange = (newPosition: MovePosition): void => {
	...
	const isSubmenuInclude = useSelect(
		(select) => {
			...
			const { getBlocksByClientId } = select(
				"core/block-editor",
			) as unknown as BlockEditorSelectors;
					...
					<ToggleControl
						...
						onChange={(newVal) => {
							if (!isMenuBlockPresent || is_menu) {
								...
								(
									dispatch("core/notices") as unknown as NoticesActions
								).createNotice(
									"error",
									__(
						...
						{parallax_obj != null && (
							<div className="itmar_title_type">
								...
								<RangeControl
									...
									max={
										getParallaxRange(parallax_obj.type, parallax_obj.unit).max
									}
									min={
										getParallaxRange(parallax_obj.type, parallax_obj.unit).min
									}
									step={
										getParallaxRange(parallax_obj.type, parallax_obj.unit).step
									}
									onChange={(newVal) =>
										setAttributes({
											parallax_obj: {
												...parallax_obj,
												scale: newVal ?? parallax_obj.scale,
											},
										})
									}
									withInputField={true}
			...
			<InspectorControls group="styles">
				...
				<BlockPlace
					...
					onDirectionChange={(position) => {
						if (!isGroupDirection(position)) return;
						if (!isMobile) {
							setAttributes({
					...
					onFreeHeightChange={(value) => {
						if (!isMobile) {
							setAttributes({
								default_val: { ...default_val, free_height: String(value) },
							});
						} else {
							setAttributes({
								mobile_val: { ...mobile_val, free_height: String(value) },
							});
						}
					}}
					onGridChange={(value) => {
						if (!isMobile) {
							setAttributes({
								default_val: {
									...default_val,
									grid_info: mergeDefined(default_val.grid_info, value),
								},
							});
						} else {
							setAttributes({
					...
					onPosValueChange={(value) => {
						if (!isMobile) {
							setAttributes({
								default_val: {
									...default_val,
									posValue: mergeDefined(default_val.posValue, value),
								},
							});
						} else {
							setAttributes({
