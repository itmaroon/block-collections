...
const StyledDiv = styled.div<{ $attr: GroupAttributes }>`
	${({ $attr }) => {
		const {
			positionType,
			isPosCenter,
		...
		//ブロックの配置
		const default_block_align = cssValueToString(
			align_prm(default_val.outer_align),
		);
		const mobile_block_align = cssValueToString(
			align_prm(mobile_val.outer_align),
		);
		//シャドースタイル
		const box_shadow_style =
			is_shadow && shadow_result ? convertToScss(shadow_result) : "";
		//ブロックの位置
		const default_block_position = cssValueToString(
			position_prm(
				isPosCenter ? isPosCenter : default_val.posValue,
				positionType,
			),
		);
		const mobile_block_position = cssValueToString(
			position_prm(
        ...
        ${is_menu &&
			...
			> div {
				${tranceform}
				>.group_contents {
					${cssValueToString(anime_comp(anime_prm))}
					${box_shadow_style};
		...
		//グリッドスタイル
		const createNthChildStyles = (
			numItems: GridElement[] | undefined,
		): string => {
			let styles = "";
			if (numItems) {
...
/**
 * フロントエンド用の純粋CSS生成関数。
 * React、renderToString、styled-componentsのランタイムを必要としない。
 */
