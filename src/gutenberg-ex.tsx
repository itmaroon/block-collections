...
type BorderValue = Record<string, unknown>;

type CoreExAttributes = {
	className?: string;
	margin_val: BoxValues;
	padding_val: BoxValues;
	scaleWidth?: string;
	scaleHeight?: string;
	isFitScale?: boolean;
	lineHeight?: number;
	...
	defaultGradient?: number;
	mobileGradient?: number;
	radius_list?: RadiusValue;
	border_list?: BorderValue;
	list_type?: string;
	align?: string;
	[key: string]: any;
};
...
type BlockEditProps = {
	...
	clientId: string;
	isValid?: boolean;
	attributes: CoreExAttributes;
	setAttributes: (attributes: Partial<CoreExAttributes>) => void;
	[key: string]: any;
};

type BlockListProps = BlockEditProps & {
	wrapperProps?: {
		className?: string;
		style?: CSSProperties;
		[key: string]: any;
...
type BlockEditorSelectors = {
	...
	getBlock: (clientId: string) => BlockInstance<CoreExAttributes> | null;
	getBlockName: (clientId: string) => string | null;
	getBlockAttributes: (clientId: string) => CoreExAttributes | null;
};
...
type BlockEditorActions = {
	...
	updateBlockAttributes: (
		clientId: string,
		attributes: Partial<CoreExAttributes>,
	) => void;
};
...
const mergeBoxValues = (current: BoxValues, value: BoxControlValue): BoxValues => ({
	...current,
	...Object.fromEntries(
...
const normalizeRadiusValue = (value: unknown): RadiusValue => {
	if (typeof value === "string") return { value };
	if (value && typeof value === "object") return value as RadiusValue;
	return {};
};

const defaultBoxValues: BoxValues = {
	top: "1em",
	left: "1em",
	right: "1em",
	bottom: "1em",
...
const withInspectorControl = createHigherOrderComponent((BlockEdit: ComponentType<any>) => {
	//スペースのリセットバリュー
	...
	const WrappedBlockEdit = (props: BlockEditProps) => {
		//モバイルの判定
		const isMobile = useIsIframeMobile();

		//最大高さ設定用の一時保管
		const [heightVal, setHeightVal] = useState<string | null>(null);
		//拡張コアブロックのクラスを持つかの判定
						...
						onChange={(value?: string) => {
							setHeightVal(value ?? null);
									...
									onChange={(val?: number) =>
										setAttributes(
											!isMobile
												? { defaultGradient: val ?? defaultGradient }
												: { mobileGradient: val ?? mobileGradient },
										)
									}
						...
						<InspectorControls group="styles">
							...
							>
								<BoxControl
									...
									onChange={(newValue: BoxControlValue) =>
										setAttributes({
											margin_val: mergeBoxValues(margin_val, newValue),
										})
									}
									units={units} // 許可する単位
								...
								<BoxControl
									...
									onChange={(newValue: BoxControlValue) =>
										setAttributes({
											padding_val: mergeBoxValues(padding_val, newValue),
										})
									}
									units={units} // 許可する単位
									allowReset={true} // リセットの可否
								...
								<>
									...
									>
										<RangeControl
											...
											step={0.1}
									onChange={(val?: number) =>
										setAttributes({ lineHeight: val ?? lineHeight })
									}
											withInputField={true}
										/>
									</PanelBody>
							...
							{(props.name === "core/list" ||
								...
								>
									...
									onChange={(newValue: Record<string, unknown> | undefined) =>
									...
									onChange={(newBrVal: unknown) =>
										setAttributes({
											radius_list: normalizeRadiusValue(newBrVal),
										})
									}
									/>
								</PanelBody>
...
const applyExtraAttributesInEditor = createHigherOrderComponent(
	(BlockListBlock: ComponentType<any>) => {
		const WrappedBlockListBlock = (props: BlockListProps) => {
			//モバイルの判定
			const isMobile = useIsIframeMobile();
			...
			const iframe = document.getElementsByName(
				"editor-canvas",
			)[0] as HTMLIFrameElement | undefined; // name属性を利用
			...
			const { nearestDesignGroupParent, moreButton, blockElement } = useSelect(
				(select) => {
					...
					const groupIds = parentIds.filter(
						(pId: string) => getBlockName(pId) === "itmar/design-group",
					);
					const parentId =
		...
		const moreButton = parentBlock.innerBlocks.find(
			(block: BlockInstance<Record<string, any>>) =>
				block.name === "itmar/design-button" &&
				Boolean(block.attributes.className?.includes("more_btn")),
		);
			...
			const { insertBlocks, removeBlock, updateBlockAttributes } =
				useDispatch("core/block-editor") as unknown as BlockEditorActions;
			const { createNotice } = useDispatch(
				"core/notices",
			) as unknown as NoticeActions;
			...
			if (hasItmarBlockClass) {
				...
				if (allowedBlocks.includes(name)) {
					if (isValid) {
						...
						let afterElement: ReactNode = null;
						...
						if (name === "core/paragraph") {
							if (moreButton) {
								...
								const removeHideName = moreClassNames.filter(
									(className: string) => className !== "more_hide",
								);
...
const applyExtraAttributesInFrontEnd = (
	props: {
		className?: string;
		style?: CSSProperties;
		[key: string]: any;
	...
	if (props.className?.match(/itmar_ex_block/)) {
		...
		if (allowedBlocks.includes(blockType.name)) {
			...
			let dataAttributes: Record<string, string> = {}; // data属性
			...
			return Object.assign(props, {
				...
				className: extraClassNames.trim(), // trim() で余分なスペースを削除
				...Object.entries(dataAttributes).reduce<Record<string, string>>((acc, [key, value]) => {
					acc[key] = value;
					return acc;
				}, {}),
			});
